...
class MenuItemEmbedding(nn.Module):
    def __init__(self, len_items, len_attributes, embedding_size):
        ...
        self.i_embedding = nn.Embedding(num_embeddings=len_items, 
                                        embedding_dim=embedding_size)
        self.a_embedding = nn.Embedding(num_embeddings=len_attributes, 
                                        embedding_dim=embedding_size)
...
class UserMenuItemEmbedding(nn.Module):
    ...
    def fit(self, x):
        ...
        m_u = torch.sum(uemb * item_ids.unsqueeze(1), dim=1, keepdim=True)  # Dot product
