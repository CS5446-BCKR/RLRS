...
class MovieGenreEmbedding(nn.Module):
    def __init__(self, len_movies, len_genres, embedding_size):
        ...
        self.m_embedding = nn.Embedding(
            num_embeddings=len_movies, embedding_dim=embedding_size
        )
        self.g_embedding = nn.Embedding(
            num_embeddings=len_genres, embedding_dim=embedding_size
        )
...
class MovieUserEmbedding(nn.Module):
    ...
    def fit(self, x):
        ...
        m_u = torch.sum(
            uemb * movie_ids.unsqueeze(1), dim=1, keepdim=True
        )  # Dot product
